...
COMMENTS = {
    ...
    '================================================================== */':
    '/* ==================================================================\n'
    '   Taking base64 and data URLs apart\n'
    '   Decoding, encoding and data-URL parsing are all written here. atob and\n'
    '   btoa are used only as the reference the self-check compares against.\n'
    '================================================================== */',
    ...
    '------------------------------------------------------------------ */':
    '/* ------------------------------------------------------------------\n'
    '   Decoding base64 (our own)\n'
    '   It reads rather than rejects, and reports anything odd with a code\n'
    '------------------------------------------------------------------ */',

    '// 6ビットしか余らない = どの1バイトにもならない。最後の1文字は使えない':
    '// Only 6 bits left over, which is no whole byte. The last character is unusable',
    '// + / - _ がどれも出てこないので区別がつかない':
    '// None of + / - _ appear, so the two alphabets cannot be told apart',

    '/* ------------------------------------------------------------------\n'
    '   base64 の符号化（自前）\n'
    '------------------------------------------------------------------ */':
    '/* ------------------------------------------------------------------\n'
    '   Encoding base64 (our own)\n'
    '------------------------------------------------------------------ */',
    ...
    '------------------------------------------------------------------ */':
    '/* ------------------------------------------------------------------\n'
    '   Reading UTF-8 (our own)\n'
    '   TextDecoder quietly replaces an invalid sequence with U+FFFD, so we read\n'
    '   it ourselves in order to say what was invalid and how\n'
    '------------------------------------------------------------------ */',
    ...
    '------------------------------------------------------------------ */':
    '/* ------------------------------------------------------------------\n'
    '   Parsing a data URL (RFC 2397, our own)\n'
    '------------------------------------------------------------------ */',

    '// mediatype を ; で分ける。最後の要素が base64 ならフラグ':
    '// Split the mediatype on ; -- a final element of base64 is the flag',
    '/* 非base64 のデータURL本体をバイト列に戻す（%XX を復号。+ は復号しない） */':
    '/* Turn a non-base64 data URL body back into bytes (decode %XX; + is not decoded) */',

    '/* ------------------------------------------------------------------\n'
    '   中身の判定（マジックナンバー）\n'
    '------------------------------------------------------------------ */':
    '/* ------------------------------------------------------------------\n'
    '   Identifying the content (magic numbers)\n'
    '------------------------------------------------------------------ */',

    '/* MIME の別名。宣言と中身を比べるときに同じものとみなす */':
    '/* MIME aliases, treated as the same when the declaration is compared with the content */',
    '/* JSON ではなかった。テキストとして続ける */':
    '/* Not JSON after all. Carry on treating it as text */',
    ...
    '/* It looks like base64 but there is no ;base64 */',
    '/* ---------- 素の base64 ---------- */': '/* ---------- Bare base64 ---------- */',
    '/* ---------- base64 まわりの共通の指摘 ---------- */':
    '/* ---------- Findings that apply to base64 in general ---------- */',
    '/* ★ 余ったビットが0でない */': '/* The leftover bits are not zero */',
    '/* ---------- 中身の判定と宣言との突き合わせ ---------- */':
    '/* ---------- Compare what the content is with what it claims to be ---------- */',
    ...
    '/* 余ったビットの図 */': '/* The diagram of the leftover bits */',
    '/* 中身 */': '/* Content */',
    '/* 指摘 */': '/* Findings */',
    ...
    '------------------------------------------------------------------ */':
    '/* ------------------------------------------------------------------\n'
    '   Self-check: compare our implementation with the browser one\n'
    '------------------------------------------------------------------ */',

    '/* 1. 符号化: 自前 vs btoa */': '/* 1. Encoding: ours vs btoa */',
    '/* 2. 復号: 自前 vs atob */': '/* 2. Decoding: ours vs atob */',
    '/* 3. 往復 */': '/* 3. Round trip */',
    '/* 4. UTF-8: 自前 vs TextDecoder / TextEncoder */':
    '/* 4. UTF-8: ours vs TextDecoder / TextEncoder */',
    '/* 5. 正規形でない base64 を、実際に atob が受け取ることの確認 */':
    '/* 5. Confirm that atob really does accept base64 that is not in canonical form */',
