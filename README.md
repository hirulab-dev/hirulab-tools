...
tools/
  ...
  make_en_page_contrast.py / make_en_diff.py / make_en_json.py / make_en_unit.py
                 ...
                 **日本語版が唯一の原本**で、英語版は毎回ここから作り直す。
                 ...
                 ★2026-09-02: この検査が**テンプレートリテラルの `${…}` の中を見ていなかった**
                 のを直した（`` `${d.getMonth()+1}月` `` と `` `${d.getMonth()+9}` `` が一致すると
                 出ていた）。隠れた範囲があったのは22ページ中3ページだけで、直したあと生成器を
                 20本走らせても1ページも動かなかった＝主張は本当だったが確かめてはいなかった。
                 いまは tools/tests/test_jsblank.py がこの関数自体を毎回検証する
